//
//  LowDensityQuestionnaireViewController.m
//  CloudPaletteBar
//
...
@interface LowDensityQuestionnaireViewController (){
...
@implementation LowDensityQuestionnaireViewController
...
- (void)viewDidLoad {
    ...
    makeTypeArray = @[@"houseXiaoqu",@"houseBuding",@"houseFangwu"];
...
-(void)searchV{
    if (self.searchView == nil) {
        ...
        self.searchView.addedPush = ^(){
            NSLog(@"点击了新增");
            addStr=@"新增";
            [searchBlock performSegueWithIdentifier:@"LowForm" sender:nil];
        };
...
-(void)searchHead{
    if ([makeTypeStr isEqualToString:@"houseXiaoqu"]) {
    ...
    }else if ([makeTypeStr isEqualToString:@"houseBuding"]){
...
-(void)kNetworkListMake2
{
    ...
    if ([makeTypeStr2 isEqualToString:@"houseXiaoqu"]) {
        [dict setObject:SelectID forKey:@"loupanId"];
    }else if ([makeTypeStr2 isEqualToString:@"houseBuding"]){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.delegate = self;
    if ([makeTypeStr isEqualToString:@"houseXiaoqu"]) {
        [cell disappearIndex:2 andArray:lowArray];
         _selectIndex = 0;
    }else if ([makeTypeStr isEqualToString:@"houseBuding"]){
        [cell disappearIndex:1 andArray:lowArray];
         _selectIndex = 1;
    }else{
        [cell disappearIndex:0 andArray:lowArray];
         _selectIndex = 2;
    }
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    [officeQuestionnaireHeatView _initHeat2:[self.baseArray objectAtIndex:section] Index:section andMakeType:makeTypeStr];
...
-(void)didSelectLocation:(BusinessTableViewCell *)aTableCell andSelectButton:(UIButton *)sender{
    ...
    switch (sender.tag) {
        case 500:{
            addStr=@"查看";
            [self performSegueWithIdentifier:@"LowForm" sender:nil];
        }
            break;
        case 501:{
            addStr=@"编辑";
...
-(void)saveObject{
    [kUserDefaults setObject:makeTypeStr forKey:@"lowDensityMakeType"];
    [kUserDefaults setObject:officeListModel.ID forKey:@"lowDensityId"];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if (![addStr isEqualToString:@"新增"]) {
        ...
        [kUserDefaults removeObjectForKey:@"lowDensityId"];
    }
    
    
    LowFormViewController *lowVC=[segue destinationViewController];
    if ([addStr isEqualToString:@"新增"]) {
        lowVC.formID=self.QID;
    }else if ([addStr isEqualToString:@"查看"]){
        lowVC.selectSee=addStr;
        lowVC.formID=self.QID;
    }
    else{
        lowVC.selectSee=addStr;
    ...
    lowVC.selectIndex=self.selectIndex;
    lowVC.BlackVC = ^(){
        backStr = @"返回";
    };
}
